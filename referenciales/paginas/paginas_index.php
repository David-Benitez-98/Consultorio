...
<?php } ?>
                                ...
                                <div class="box-body no-padding">
                                    <div class="row">
                                        <div class="col-lg-12 col-md-12 col-xs-12">
                                            ...
                                            <?php
                                            $ciudades = consultas::get_datos("select * from ciudad where ciu_descri ilike '%" . (isset($_REQUEST['buscar']) ? $_REQUEST['buscar'] : "") . "%' order by ciu_cod");
                                            if (!empty($ciudades)) {
                                                ?>
    ...
    <?php foreach ($ciudades as $ciu) { ?>
                                                                <tr>
                                                                    <td data-title="Descripcion"><?php echo $ciu['ciu_descri']; ?></td>
                                                                    <td data-title="Estado"><?php echo $ciu['ciu_estado']; ?></td>
                                                                    <td data-title="Acciones" class="text-center">
        <?php if ($ciu['ciu_estado'] == 'ACTIVO') { ?>
                                                                            <a onclick="editar('<?php echo $ciu['ciu_cod'] . "_" . $ciu['ciu_descri']; ?>')"       
                                                                            ...
                                                                            <a onclick="deshabilitar('<?php echo $ciu['ciu_cod'] . "_" . $ciu['ciu_descri']; ?>')" 
        ...
        <?php } else { ?>
                                                                            <a onclick="activar('<?php echo $ciu['ciu_cod'] . "_" . $ciu['ciu_descri']; ?>')" 
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title">Registrar Ciudades</h4>
                        </div>
                        <form action="ciudad_control.php" method="post" accept-charset="utf-8" class="form-horizontal">
                            ...
                            <input name="vciu_cod" value="0" type="hidden"/>
                            <div class="box-body">
                                <div class="form-group">
                                    <label class="col-sm-2 control-label">Ciudad</label>
                                    <div class="col-sm-5 col-lg-5 col-xs-5 col-md-5">
                                        <input type="text" class="form-control" name="vciu_descri" required=""/>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form action="ciudad_control.php" method="post" accept-charset="utf-8" class="form-horizontal">
                            ...
                            <input id="cod" name="vciu_cod" type="hidden"/>
                            <div class="box-body">
                                <div class="form-group">
                                    <label class="col-sm-2 control-label">Descripcion</label>
                                    <div class="col-sm-12 col-lg-12 col-xs-12 col-md-12">
                                        <input id="descri" type="text" class="form-control" name="vciu_descri" required=""/>
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer">
...
<?php require '../../menu2/js_lte.ctp'; ?><!--ARCHIVOS JS-->
        ...
        <script>
            ...
            function deshabilitar(datos) {
                var dat = datos.split("_");
                $('#si').attr('href', 'ciudad_control.php?vciu_cod=' + dat[0] + '&vciu_descri=' + dat[1] + '&accion=3');
                $('#confirmacion').html('<span class="glyphicon glyphicon-warning-sign"></span> \n\
                                Desea inhabilitar la ciudad <i><strong>' + dat[1] + '</strong></i>');
            }
            function activar(datos) {
                var dat = datos.split("_");
                var vciu_cod = dat[0];
                var vciu_descri = dat[1];
                var vciu_estado = dat[2];
                // Configurar la URL para enviar los datos al script ciudad_control.php
                var url = 'ciudad_control.php?accion=4&vciu_cod=' + vciu_cod + '&vciu_descri=' + vciu_descri + '&vciu_estado=' + vciu_estado;

                // Actualizar el enlace 'si' con la URL correcta
                $('#si_activar').attr('href', url);

                // Configurar el mensaje de confirmación
                var mensaje = 'Desea activar la ciudad <strong>' + vciu_descri + '</strong>?';
