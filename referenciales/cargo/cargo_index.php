...
<?php } ?>
                            <div class="box box-primary">
                                <div class="box-header">
                                    ...
                                    <div class="box-tools">
                                        ...
                                        <a href="cargo_print.php" class="btn btn-default btn-sm" data-title="Imprimir" rel="tooltip" data-placement="left" target="print">
    ...
    <?php foreach ($cargos as $car) { ?>
                                                                <tr>
                                                                    ...
                                                                    <td data-title="Estado"><?php echo $car['car_estado']; ?></td>
                                                                    <td data-title="Acciones" class="text-center">
        <?php if ($car['car_estado'] == 'ACTIVO') { ?>
                                                                            <a onclick="Editar('<?php echo $car['car_cod'] . "_" . $car['car_descri']; ?>')"       
                                                                            ...
                                                                            <a onclick="Deshabilitar('<?php echo $car['car_cod'] . "_" . $car['car_descri']; ?>')" 
        ...
        <?php } else { ?>
                                                                            <a onclick="Activar('<?php echo $car['car_cod'] . "_" . $car['car_descri']; ?>')" 
                                                                               class="btn btn-xs btn-xs" role="button" data-title="Activar"
...
<?php require '../../menu2/footer_lte.ctp'; ?><!--ARCHIVOS JS--> 
                            ...
                            <div class="box-body">
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-5 col-lg-5 col-xs-5 col-md-5">
                                        <input type="text" class="form-control" name="vcar_descri" required=""/>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="modal fade" id="editar" role="dialog">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">x Cerrar</button>
                            <h4 class="modal-title">Editar Datos de Cargo</h4>
                        </div>
                        <form action="cargo_control.php" method="post" accept-charset="utf-8" class="form-horizontal">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" class="btn btn-warning">
                                    <i class="fa fa-meh-o"></i> Actualizar</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        <script>
            ...
            function deshabilitar(datos) {
                ...
                $('#confirmacion').html('<span class="glyphicon glyphicon-warning-sign"></span> \n\
                                Desea inhabilitar el cargo <i><strong>' + dat[1] + '</strong></i>');
            }
            function activar(datos) {
                var dat = datos.split("_");
                var vcar_cod = dat[0];
                var vcar_descri = dat[1];
                var vcar_estado = dat[2];
                // Configurar la URL para enviar los datos al script ciudad_control.php
                var url = 'cargo_control.php?accion=4&vcar_cod=' + vcar_cod + '&vcar_descri=' + vcar_descri + '&vcar_estado=' + vcar_estado;

                // Actualizar el enlace 'si' con la URL correcta
                $('#si_activar').attr('href', url);

                // Configurar el mensaje de confirmación
                var mensaje = 'Desea activar el cargo <strong>' + vcar_descri + '</strong>?';
