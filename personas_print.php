...
//COLOR DE TABLA
        ...
        $pdf->Cell(40,2,'APELLIDO', 1, 0, 'C', 1);
        ...
        if (!empty(isset($_REQUEST['vdesde']))&& !empty(isset($_REQUEST['vhasta']))) {
            if ($_REQUEST['opcion']=='1') {
                $clientes = consultas::get_datos("select * from clientes "
                        . "where cli_cod between ".$_REQUEST['vdesde']." and "
                        .$_REQUEST['vhasta']." order by cli_cod");
            } else {
                $clientes = consultas::get_datos("select * from clientes "
                        . "where cli_nombre between '".$_REQUEST['vdesde']."' and '"
                        .$_REQUEST['vhasta']."'");
            }
         } else {
        ...
        foreach ($clientes as $cliente) {
            $pdf->Cell(20,5,$cliente['cli_cod'], 1, 0, 'C', 1);
            $pdf->Cell(30,5,$cliente['cli_ci'], 1, 0, 'C', 1);
            $pdf->Cell(30,5,$cliente['cli_nombre'], 1, 0, 'C', 1);
            $pdf->Cell(40,2,$cliente['cli_apellido'], 1, 0, 'C', 1);
            $pdf->Cell(40,5,$cliente['cli_telefono'], 1, 0, 'C', 1);
            $pdf->Cell(40,5,$cliente['cli_direcc'], 1, 0, 'C', 1);
            $pdf->Ln();
        }
