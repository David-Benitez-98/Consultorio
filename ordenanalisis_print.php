...
$pdf->SetTitle('ORDEN DE ANALISIS DETALLE');
$pdf->SetSubject('Prototipo de Impresión para Orden de Analisis Detalle');
...
$pdf->SetXY(30, 70);
$pdf->Cell(0, 20, 'ORDEN DE ANALISIS ', 0, 1, 'C');

// Obtener datos de la vista v_ordenestudiosdetalle (ajusta la consulta según tu estructura de base de datos)
$ordenEstudiosDetalle = consultas::get_datos("SELECT * FROM v_ordenanalisisdetalle WHERE pac_cod = $pac_cod");
...
if (!empty($ordenEstudiosDetalle)) {
    ...
    // Iterar sobre los datos y agregarlos de manera personalizada
        ...
        $pdf->MultiCell(0, 10, 'Fecha: ' . $ordenDetalle['oa_fecha'], 0, 'L');
        $pdf->MultiCell(0, 10, 'Tipo de Analisis: ' . $ordenDetalle['tipooa_descri'], 0, 'L');
...
// Salida al navegador
$pdf->Output('informe_orden_analisis_detalle.pdf', 'I');
?>
