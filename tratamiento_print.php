...
$pdf->SetTitle('TRATAMIENTO Y SUS DETALLES');
$pdf->SetSubject('Prototipo de Impresión para Tratamientos ');
$pdf->SetKeywords('TCPDF, PDF, tratamiento médico, informe médico');
...
$pdf->SetXY(30, 70);
$pdf->Cell(0, 20, 'TRATAMIENTOS ', 0, 1, 'C');
// Obtener datos de la consulta solo para el paciente seleccionado
$cabeceras = consultas::get_datos("SELECT * FROM v_detalletratamiento WHERE pac_cod = $pac_cod");
...
if (!empty($cabeceras)) {
    foreach ($cabeceras as $cabecera) {
        ...
        $pdf->Cell(0, 6, "Fecha: " . $cabecera['tra_fecha'], 0, 1, 'L');
        $pdf->Cell(0, 6, "Tipo Tratamiento: " . strtoupper($cabecera['tipotra_descri']), 0, 1, 'L');
        $pdf->Cell(0, 6, "Observacion: " . $cabecera['detalle_observ'], 0, 1, 'L');
        $pdf->Cell(0, 6, "Fecha Inicio: " . $cabecera['tra_fechainicio'], 0, 1, 'L');
        $pdf->Cell(0, 6, "Fecha Fin: " . $cabecera['tra_fechafin'], 0, 1, 'L');
    ...
    $pdf->Cell(0, 10, 'No se encontraron registros de tratamientos para este paciente.', 0, 1, 'C');
}
$pdf->Output('reporte_tratamiento.pdf', 'I');
?>
