...
$cabeceras = consultas::get_datos("SELECT * FROM v_diagnosticodetalle WHERE pac_cod = $pac_cod");
...
if (!empty($cabeceras)) {
    foreach ($cabeceras as $cabecera) {
        ...
        $pdf->Cell(0, 6, "Fecha: " . $cabecera['diag_fecha'], 0, 1, 'L');
        $pdf->Cell(0, 6, "Diagnostico Enfermedad: " . strtoupper($cabecera['enfe_descri']), 0, 1, 'L');
        $pdf->Cell(0, 6, "Tipo de Enfermedad: " . $cabecera['tipoenfe_descri'], 0, 1, 'L');
        $pdf->Cell(0, 6, "Observaciones: " . $cabecera['diag_descri'], 0, 1, 'L');
